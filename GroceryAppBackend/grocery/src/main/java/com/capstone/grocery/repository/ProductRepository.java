...
public interface ProductRepository extends MongoRepository<Product, String> {
    ...
    @Query("{ $or: [{ id: { $regex: ?0, $options: 'i' } },{ desc: { $regex: ?0, $options: 'i' } },{ sku_max_quantity: { $regex: ?0, $options: 'i' } },{ pack_desc: { $regex: ?0, $options: 'i' } },{ weight: { $regex: ?0, $options: 'i' } },{ absolute_url: { $regex: ?0, $options: 'i' } },{ usp: { $regex: ?0, $options: 'i' } },{ variableWeight: { $regex: ?0, $options: 'i' } },]}")
